...
Global $bFirstAttackClick ; Changed to Global to match usage across functions
Global $AttackCount
Global $iCounter = 0 
...
Func DoAttackBB()
    ...
    If $g_iBBAttackCount = 0 Then
        While PrepareAttackBB($AttackCount)
            ...
            If $AttackCount > 50 Then
                SetLog("Already Attack 50 times, continue next time", $COLOR_INFO)
                $iCounter = 0
                ExitLoop
            EndIf
    ...
    Else
        ...
        If $g_iBBAttackCount = 1 Then
            $g_iBBAttackCountFinal = Random(40, 50, 1)
...
Func EndBattleBB() ; Find if battle has ended and click okay
    Local $bRet = True, $bBattleMachine = True, $bokBomber = True
    ...
    For $i = 1 To 200
        ...
        If $sTmpDamage = 100 Then
            ...
            If _Sleep(3000) Then Return ; Add some delay for troops making some damage
            $sTmpDamage = 0
            $bBattleMachine = True
            $bBomber = True
        EndIf
...
Func AttackBB($aBBAttackBar = True)
    ...
    While Not $bTroopsDropped
        ...
        If $aBBAttackBar = "" Then
            ...
            SetLog("Waiting for end of battle.", $COLOR_INFO)
            $bTroopsDropped = True
            RestartCOCAfterDeployAllTroops()
        EndIf
    WEnd
...
Func DeployBBTroop($sName, $x, $y, $iAmount, $ai_AttackDropPoints)
    ...
    PureClick($x + $iRandomX, $y + $iRandomY) ; select troop
    ...
    For $j = 0 To $iAmount - 1
        ...
        SetLog("Drop Point 1: " & $iPixel2[0] & ", " & $iPixel2[1], $COLOR_DEBUG) ; check drop points
        $iPixel2[0] = $iPixel2[0] + $iRandomX
        $iPixel2[1] = $iPixel2[1] + $iRandomY
        SetLog("Drop Point 2: " & $iPixel2[0] & ", " & $iPixel2[1], $COLOR_DEBUG) ; check drop points
        If $bFirstAttackClick Then
            IsClickOnPotions($iPixel2[0], $iPixel2[1]) ; Fixed: Use $iPixel2 instead of $iPixel
