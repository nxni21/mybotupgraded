...
Func DropTroopFromINI($sDropVectors, $iStartIndex, $iEndIndex, $aiIndexArray, $iMinQuantity, $iMaxQuantity, $sTroopName, $delayPointmin, $delayPointmax, $delayDropMin, $delayDropMax, $sleepafterMin, $sleepAfterMax, $bDebug = False)
	...
	Switch $iTroopIndex
		...
		Case $eRvSpell
			If Not $g_abAttackUseReviveSpell[$g_iMatchMode] Then $bUseSpell = False
		Case $ePSpell
				...
				If $index <= UBound(Execute("$" & Eval("vector" & $j))) Then
					...
					Switch $iTroopIndex
						Case $eBarb To $eDruid ; drop normal/super troops
							...
							Else
								AttackClickCSV($pixel[0], $pixel[1], $qty2, $delayPoint, $delayDropLast, "#0666")
							EndIf
						Case $eKing
							If $bDebug Then
								SetLog("dropHeroes(" & $pixel[0] & ", " & $pixel[1] & ", " & $troopPosition & ", -1, -1, -1, -1) ")
							Else
								dropHeroes($pixel[0], $pixel[1], $troopPosition, -1, -1, -1, -1) ; was $g_iKingSlot, Slot11+
							EndIf
						Case $eQueen
							If $bDebug Then
								SetLog("dropHeroes(" & $pixel[0] & ", " & $pixel[1] & ",-1," & $troopPosition & ", -1, -1, -1) ")
							Else
								dropHeroes($pixel[0], $pixel[1], -1, $troopPosition, -1, -1, -1) ; was $g_iQueenSlot, Slot11+
							EndIf
						Case $ePrince
							If $bDebug Then
								SetLog("dropHeroes(" & $pixel[0] & ", " & $pixel[1] & ",-1, -1, " & $troopPosition & ", -1, -1) ")
							Else
								dropHeroes($pixel[0], $pixel[1], -1, -1, $troopPosition, -1, -1) ; was $g_iPrinceSlot, Slot11+
							EndIf
						Case $eWarden
							If $bDebug Then
								SetLog("dropHeroes(" & $pixel[0] & ", " & $pixel[1] & ", -1, -1, -1" & $troopPosition & ", -1) ")
							Else
								dropHeroes($pixel[0], $pixel[1], -1, -1, -1, $troopPosition, -1) ; was $g_iWardenSlot, Slot11+
							EndIf
						Case $eChampion
							If $bDebug Then
						...
						Case $eLSpell To $eOgSpell
							If $bDebug Then
								SetLog("Drop Spell AttackClickCSV( " & $pixel[0] & ", " & $pixel[1] & " , " & $qty2 & ", " & $delayPoint & ",#0666)")
							Else
								If $eLSpell Then 
									Local $counter1 = 0, $xy1 = $pixel[0] + $pixel[1], $temp1 = 0
									
									; display air def location
									For $i = 0 To UBound($ADLoc) - 1									
										If $ADLoc[$i] = $xy1 Then
											$counter1 += 1
										Else 
											If $ADLoc[$i] > $xy1 Then 
									...
									; populate air def location
									For $i = 0 To UBound($ADLoc) - 1
										If ($ADLoc[$i] = 0 And $ADLoc[0] <> $xy1 And $ADLoc[1] <> $xy1 And $ADLoc[2] <> $xy1 And $ADLoc[3] <> $xy1 And $ADLoc[4] <> $xy1) Then 
												$ADLoc[$i] = $xy1
												ExitLoop
										EndIf
									Next

									; Drop lightning spell
									If $counter1 = 0 Then 	
										SetLog("Drop Lightning: " & $pixel[0] & ", " & $pixel[1] & ", " & $qty2, $COLOR_INFO)
										AttackClickCSV($pixel[0], $pixel[1], $qty2, $delayPoint, $delayDropLast, "#0667")
									Else 
										$qty2 = 0
									EndIf
								Else 
